...
import com.dingler.mapper.UserControlMessageMapper;
...
public class UserControlMessageImpl implements UserControlMessage {
    ...
    @Autowired
    private UserControlMessageMapper userControlMessageMapper;
    ...
    public void saveMessage(String currentdate, String userip, String deviceip, int deviceport, String river,
        ...
        userControlMessageMapper.saveMessage(currentdate, userip, deviceip, deviceport, river, deviceplace, username, devicename, controlmessage, feedbackmessage);
    }
    @Override
    public List<UserMessageVo> selectMessage() {
        return userControlMessageMapper.selectMessage();
    }
    public JSONObject inqueryriver(String beforedate, String afterdate, String rivername, int currentpage, int pagesize) {
        ...
        Object data;
        int datacount = userControlMessageMapper.inquerypagecount(mymap);
        int pagecount = datacount / 20 + 1;
        jsonObject.put("datacount", datacount);
        jsonObject.put("pagecount", pagecount);
        data = (JSON.toJSON(userControlMessageMapper.inquerydevicelog(mymap)));
